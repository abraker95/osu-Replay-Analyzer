...
double Pattern2Reading(std::tuple<int, int, int, int> _p1, std::tuple<int, int, int, int> _p2, std::tuple<int, int, int, int> _p3, double _ARms, double _CSpx)
{
	...
	/*double ctrlReq = CtrlPatternReq(_p1, _p2, _p3, _CSpx);
	double jumpReq = JumpPatternReq(_p1, _p2, _p3, _CSpx);
	//std::cout << ctrlReq * jumpReq << std::endl;
...
double getReadingSkill(Beatmap* _beatmap, int _time)
{
	double reading = 0.0;
	...
	else
	{
		...
		reading = Pattern2Reading(pattern[2], pattern[1], pattern[0], 0.0, CS2px(_CS));
